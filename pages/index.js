...
// arriba de pages/index.js
import i18next from "i18next";
const cvHref = () =>
  (i18next.language || "es").startsWith("en") ? "/cv/en.pdf" : "/cv/es.pdf";
...
<header className="sticky top-0 z-40 backdrop-blur border-b-2" style={{borderColor:'var(--ink)'}}>
  <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-4">
    {/* BRAND: no wrap + no shrink */}
    <a href="#top" className="flex items-center gap-3 shrink-0">
      <div className="w-9 h-9 card grid place-items-center font-black">LF</div>
      <span className="font-extrabold tracking-tight whitespace-nowrap">Luis Ferrer Forteza</span>
    </a>
    ...
    {/* CONTROLES: no shrink para reservar espacio */}
    <div className="flex gap-3 shrink-0">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <a className="btn" href={cvHref()} download>
                <IconDownload />
                {t("hero.ctaCV")}
              </a>
            </div>
      ...
      <Section id="skills" title={t("skills.title")}>
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                { label: "SQL (Advanced)", icon: "icons/sql.svg" },
                { label: "MongoDB (Advanced)", icon: "icons/mongodb.svg" },
                { label: "Redis (Proficient)", icon: "icons/redis.svg" },
              ].map((s) => (
                <SkillBadge key={s.label} label={s.label} icon={s.icon} />
              ))}
            </div>
          ...
          <div className="card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                { label: "Agile/Scrum", icon: "icons/agile.svg" },
                { label: "Linux", icon: "icons/linux.svg" },
                { label: "Docker", icon: "icons/docker.svg" },
                { label: "Jupyter Notebooks", icon: "icons/jupyter.svg" },
              ].map((s) => (
                <SkillBadge key={s.label} label={s.label} icon={s.icon} />
              ))}
            </div>
          ...
          <div className="card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                { label: "Python (Advanced)", icon: "icons/python.svg" },
                { label: "Deep Learning", icon: "icons/dl.svg" },
                { label: "Machine Learning", icon: "icons/ml.svg" },
                { label: "CNN", icon: "icons/cnn.svg" },
              ].map((s) => (
                <SkillBadge key={s.label} label={s.label} icon={s.icon} />
              ))}
            </div>
      ...
      <Section id="contact" title={t("contact.title")}>
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <a className="card p-6 block btn" href="/cv/en.pdf" download>
            <IconDownload />
            {t("contact.cv")}
          </a>
          <a className="card p-6 block btn" href="/cv/es.pdf" download>
            <IconDownload />
            {t("contact.cvEs")}
          </a>
        </div>
